...
[_tornadasource] spawn {
	...
	[_tuner_tor] remoteExec ["NAT_fnc_tunetTornado",0];
	//sleep 10 + random 60;
	sleep 3;
	};
};
...
_xdest = _tornado_dest select 0;
_ydest = _tornado_dest select 1;

_xstart= _tornado_start select 0;
_ystart= _tornado_start select 1;
